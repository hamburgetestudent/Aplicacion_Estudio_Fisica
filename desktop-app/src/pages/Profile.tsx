...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    // Cargar Perfil
    const storedProfile = safeParse('userProfile', {
      name: 'Estudiante de Física',
      avatar: 'avatar_1',
    });
          ...
          <div className="flex items-center justify-center md:justify-start gap-3 mb-2">
            {isEditingName ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => {
                    saveProfile({ ...profile, name: tempName });
      ...
      <div>
        <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
          ...
          <span className="text-sm text-gray-500 font-normal">
            ({unlocked.length} / {ACHIEVEMENTS_DATA.length})
          </span>
        </h2>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-4 reltive z-10">
                  <div
                    className={`p-3 rounded-xl ${isUnlocked ? 'bg-gray-700/50' : 'bg-gray-800'}`}
                  >
                    {isUnlocked ? (
                  ...
                  <div>
                    <h3
                      className={`font-bold text-lg mb-1 ${isUnlocked ? 'text-white' : 'text-gray-500'}`}
                    >
                      {ach.title}
                    </h3>
                    ...
                    {isUnlocked && (
                      <div className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-yellow-500 bg-yellow-500/10 px-2 py-1 rounded">
                        <Star size={12} fill="currentColor" />+{ach.xpReward} XP
                      </div>
                    )}
                  </div>
                </div>
